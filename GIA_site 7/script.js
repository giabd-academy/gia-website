...
document.querySelectorAll('.hvideo,.videowrap,.vstage').forEach(function(w){
  var v=w.querySelector('video'); if(!v) return;
  var play=w.querySelector('.play'), un=w.querySelector('.unmute');
  function sync(){ if(play) play.textContent=v.paused?'▶':'❚❚'; }
  if(play) play.addEventListener('click',function(){
    if(v.paused){ v.play().catch(function(){ v.muted=true; v.play(); }); } else { v.pause(); }
  });
