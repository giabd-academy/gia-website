...
document.querySelectorAll('.card,.ccard,.panel,.founder,.pricebox,.sec,.tlrow,.faq details,.feewrap,.videowrap')
...
function fuji(){
  ...
  try{
    ...
    var d1=new THREE.DirectionalLight(0xd4a531,1.15); d1.position.set(-5,5,5); scene.add(d1);
    var d2=new THREE.DirectionalLight(0x7fa3d6,0.85); d2.position.set(5,3,3); scene.add(d2);

    var grp=new THREE.Group(); scene.add(grp);

    /* glowing sun disc */
    var sun=new THREE.Mesh(new THREE.CircleGeometry(1.55,56),
      new THREE.MeshBasicMaterial({color:0xc0392b,transparent:true,opacity:0.9}));
    sun.position.set(2.5,3.2,-5); grp.add(sun);
    var halo=new THREE.Mesh(new THREE.RingGeometry(1.6,2.5,56),
      new THREE.MeshBasicMaterial({color:0xc0392b,transparent:true,opacity:0.16,side:THREE.DoubleSide}));
    ...
    var body=new THREE.Mesh(new THREE.ConeGeometry(3.4,3.7,7,1,true),
      new THREE.MeshStandardMaterial({color:0x16294a,flatShading:true,roughness:0.85,metalness:0.15,
    ...
    var wire=new THREE.Mesh(new THREE.ConeGeometry(3.44,3.74,7,5,true),
      new THREE.MeshBasicMaterial({color:0xd4a531,wireframe:true,transparent:true,opacity:0.34}));
    wire.position.y=0.15; grp.add(wire);

    /* snow cap */
    var cap=new THREE.Mesh(new THREE.ConeGeometry(1.2,1.3,7,1,true),
      new THREE.MeshStandardMaterial({color:0xf7f3ea,flatShading:true,roughness:0.55,
        emissive:0xd4a531,emissiveIntensity:0.2,side:THREE.DoubleSide}));
    ...
    for(var i=0;i<3;i++){
      var rg=new THREE.Mesh(new THREE.TorusGeometry(2.6+i*0.55,0.012,8,90),
        new THREE.MeshBasicMaterial({color:i===1?0xc0392b:(i===2?0xe8c87a:0xd4a531),transparent:true,opacity:0.5-i*0.1}));
    ...
    var pts=new THREE.Points(pg,new THREE.PointsMaterial({color:0xe8d9ae,size:0.045,transparent:true,opacity:0.6}));
    grp.add(pts);
...
document.querySelectorAll('.videowrap,.vstage').forEach(function(w){
